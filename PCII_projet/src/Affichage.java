...
public class Affichage extends JPanel {
	...
	private void drawRoad(Graphics g) {
		...
		float curvDiff = (targetCurvature - road.curvature);
		road.curvature += curvDiff;

		while (i < road.sizeR - 1) {
			Line l = road.lines.get(i);
			Line p = road.lines.get(i + 1);
			float persp = l.project();
			float midddlePoint = (float) (road.middlePoint );
			...
			Color grass = Math.sin(70.0f * Math.pow(1.0f - persp, 3) + vehicule.distance * 0.1f) > 0.0f
					? new Color(16, 200, 16)
					: new Color(0, 154, 0);
			Color clip = Math.sin(80.0f * Math.pow(1.0f - persp, 3) + vehicule.distance) > 0.0f
	...
	private void setSpeed() {
		this.speed.setText("Speed:" + new DecimalFormat("##.##").format(this.vehicule.speed) + "Distance:"
				+ new DecimalFormat("##.##").format(this.vehicule.distance));
	}
